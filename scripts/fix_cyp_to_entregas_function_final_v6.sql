...
BEGIN
    ...
    -- CORRECCIÓN: NEW.validated se compara como BOOLEANO
    IF NEW.cyp_status = 'completado' AND (OLD.cyp_status IS NULL OR OLD.cyp_status != 'completado')
       AND NEW.photo_360_status = 'completado'
       AND NEW.validated = TRUE THEN -- ¡CORREGIDO: Comparación con BOOLEANO TRUE!
        
        RAISE NOTICE '🚗 Procesando vehículo % con CyP completado, 360 completado y validado (BOOLEANO)', NEW.license_plate;
