-- =====================================================
-- ARREGLAR VERIFICACIÓN DE DUPLICADOS
-- =====================================================

-- 1. Crear función que verifique duplicados
...
BEGIN
    RAISE NOTICE 'Iniciando procesamiento con verificación de duplicados...';
    ...
    LOOP
        ...
        -- Procesar vehículos que cumplan criterios básicos
        ...
        LOOP
            -- VERIFICAR SI YA EXISTE LA MATRÍCULA (EVITAR DUPLICADOS)
            IF NOT EXISTS (
                ...
                END;
            ELSE
                -- LA MATRÍCULA YA EXISTE, SALTAR
                skipped_count := skipped_count + 1;
                RAISE NOTICE 'Vehículo ya existe (saltado): %', vehicle_record."Matrícula";
            END IF;
...
-- 3. Probar la función con verificación de duplicados
SELECT '=== PROCESANDO CON VERIFICACIÓN DE DUPLICADOS ===' as info;
...
SELECT 
    'Matrículas únicas en duc_scraper' as info,
    COUNT(DISTINCT "Matrícula") as total
FROM duc_scraper 
WHERE "Matrícula" IS NOT NULL
UNION ALL
SELECT 
    'Matrículas únicas en nuevas_entradas' as info,
    COUNT(DISTINCT license_plate) as total
FROM nuevas_entradas; 