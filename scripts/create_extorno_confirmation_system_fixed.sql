...
DECLARE
    ...
    token_uuid uuid;
BEGIN
    -- Intentar convertir el texto a UUID
    BEGIN
        token_uuid := token_to_confirm::uuid;
    EXCEPTION WHEN invalid_text_representation THEN
        RETURN QUERY SELECT 
            false as success,
            'Token no válido - formato incorrecto' as message,
            ''::text as matricula,
            ''::text as cliente,
    ...
    -- Buscar el extorno usando el UUID convertido
    SELECT * INTO extorno_record
    FROM public.extornos 
    WHERE confirmation_token = token_uuid
