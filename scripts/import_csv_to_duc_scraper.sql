-- =====================================================
-- SISTEMA DE IMPORTACIÓN CSV A DUC_SCRAPER
-- =====================================================
-- Fecha: $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")
-- Descripción: Sistema para importar datos CSV a duc_scraper
-- =====================================================

-- Función para importar CSV a duc_scraper
CREATE OR REPLACE FUNCTION import_csv_to_duc_scraper(
    csv_data JSONB,
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            chasis_value := row_data->>'Chasis';
            concesionario_value := row_data->>'Concesionario';
            
            -- Verificar si ya existe (mismo Chasis y Concesionario)
            SELECT id INTO existing_id 
            FROM duc_scraper 
            WHERE "Chasis" = chasis_value 
            AND "Concesionario" = concesionario_value;
            
            IF existing_id IS NOT NULL THEN
                -- Actualizar registro existente
                UPDATE duc_scraper SET
                    "ID Anuncio" = row_data->>'ID Anuncio',
...
BEGIN
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'SISTEMA DE IMPORTACIÓN CSV CREADO';
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'Función creada: import_csv_to_duc_scraper()';
    RAISE NOTICE 'Función creada: get_duc_scraper_stats()';
    RAISE NOTICE '';
    RAISE NOTICE 'USO:';
    RAISE NOTICE 'SELECT * FROM import_csv_to_duc_scraper(csv_data, filename);';
