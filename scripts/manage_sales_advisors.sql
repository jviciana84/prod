...
BEGIN
    ...
    SELECT id INTO user_id FROM auth.users WHERE email = advisor_email;
    ...
    IF EXISTS (SELECT 1 FROM user_roles WHERE user_id = user_id::text AND role_id = sales_role_id) THEN
    ...
    VALUES (user_id, advisor_full_name, advisor_alias, advisor_phone, advisor_email, NOW())
    ...
    VALUES (user_id::text, sales_role_id, NOW());
...
-- Ejemplos de cómo añadir asesores (CAMBIAR POR DATOS REALES)
