-- Crear tabla para solicitudes de Permiso de Circulación (VERSIÓN CORREGIDA)
-- Primero verificar el tipo de datos de la columna id en entregas
DO $$
DECLARE
    id_type text;
BEGIN
    -- Verificar el tipo de datos de la columna id en entregas
    SELECT data_type INTO id_type
    FROM information_schema.columns 
    WHERE table_name = 'entregas' AND column_name = 'id';
    
    RAISE NOTICE 'Tipo de datos de id en entregas: %', id_type;
END $$;

-- Crear tabla para solicitudes de Permiso de Circulación
CREATE TABLE IF NOT EXISTS circulation_permit_requests (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    entrega_id TEXT REFERENCES entregas(id) ON DELETE CASCADE, -- Cambiado a TEXT para compatibilidad
