...
async function resolveAsesorName(supabase: any, asesorAlias: string | null | undefined): Promise<string> {
    ...
    if (salesData) {
      ...
      const salesWithResolvedNames = await Promise.all(
        salesData.map(async (item) => {
          const resolvedAdvisor = await resolveAsesorName(supabase, item.advisor)
          return {
            ...item,
    ...
    if (entregasData) {
      const entregasWithResolvedNames = await Promise.all(
        entregasData.map(async (item) => {
          const resolvedAsesor = await resolveAsesorName(supabase, item.asesor)
          return {
            ...item,
