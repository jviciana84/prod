...
import { Bell, Car, CheckCircle, Shield } from "lucide-react"
import { toast } from "sonner"

export default function TestNewSalePage() {
  ...
  const testNewSaleNotification = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch("/api/notifications/new-sale", {
        method: "POST",
  ...
  const simulateNewSaleTrigger = async () => {
    setIsLoading(true)
    try {
      // Simular inserción en sales_vehicles que activaría el trigger
      const response = await fetch("/api/test/simulate-new-sale", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          sale_price: formData.sale_price || 45000,
          discount: formData.discount || 2000,
          client_name: formData.client_name || "Cliente Prueba"
        })
      })
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold mb-2">Test Ventas Nuevas</h1>
        <p className="text-gray-600">Prueba las notificaciones automáticas de ventas nuevas</p>
      </div>
      ...
      <div className="grid gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Completa los datos para simular una nueva venta</CardDescription>
          </CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-4">
              <Button 
                onClick={testNewSaleNotification}
                disabled={isLoading}
                className="flex-1"
              >
                {isLoading ? "Enviando..." : (
              ...
              <Button 
                onClick={simulateNewSaleTrigger}
                disabled={isLoading}
                variant="outline"
                className="flex-1"
              >
            ...
            <div className="text-sm text-gray-500 space-y-2">
              ...
              <p><strong>Simular Trigger:</strong> Simula la inserción en sales_vehicles que activaría el trigger automático</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <ol className="list-decimal pl-5 space-y-1">
              <li>Se registra una nueva venta en Gestión de Ventas</li>
              <li>Se inserta un nuevo registro en <code>sales_vehicles</code></li>
            ...
            <p className="mt-2"><strong>Información incluida:</strong> Matrícula, modelo, asesor, precio de venta, descuento y cliente</p>
          </CardContent>
        </Card>
      </div>
    </div>
