...
export function SalesRanking({ className }: SalesRankingProps) {
  ...
  const loadRankingData = async () => {
    try {
      setLoading(true)
      console.log("🔍 Cargando ranking de ventas...")

      // Obtener ranking del mes actual
      const currentDate = new Date()
      const firstDayOfMonth = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1).toISOString()

      // Obtener perfiles para avatares y full_name
     ...
     `)
      ...
      console.log("📊 Sales data para ranking de ventas:", salesData)
      console.log("⚠️ Error en sales:", error)

      if (!error && salesData && salesData.length > 0) {
        const advisorCounts: Record<string, { name: string; avatar_url?: string; count: number; totalAmount: number }> =
          {}

        salesData.forEach((sale) => {
          let profileInfo = null
          let advisorKey = "unknown" // Default key for unknown advisors
          ...
          advisorCounts[advisorKey].totalAmount += sale.price || 0
        })
        ...
        console.log("🏆 Ranking de ventas calculado:", ranking)
        setCurrentRanking(ranking)
        setDebugInfo(`Ventas totales: ${salesData.length}, Asesores: ${Object.keys(advisorCounts).length}`)
      } else {
        console.log("❌ No hay datos de ventas este mes")
        setDebugInfo("Sin datos de ventas este mes")
        setCurrentRanking([])
      }
     ...
     `)
      ...
      console.error("💥 Error cargando ranking de ventas:", error)
  ...
  const loadHistoricalWinners = async (
    ...
    try {
      console.log("📅 Cargando historial de ganadores mensuales...")
      ...
      for (let i = 0; i < 6; i++) {
        ...
        console.log(
          `📊 Analizando mes: ${targetDate.getFullYear()}-${targetDate.getMonth() + 1} (del ${firstDay} al ${lastDay})`,
        )
        ...
        if (monthSalesError) {
          console.error(
            `❌ Error fetching sales data for ${targetDate.getMonth() + 1}/${targetDate.getFullYear()}:`,
            monthSalesError,
          )
          winners.push({
        ...
        console.log(
          `Fetched ${monthSalesData?.length || 0} sales for ${targetDate.getMonth() + 1}/${targetDate.getFullYear()}`,
        )

        if (monthSalesData && monthSalesData.length > 0) {
          // Contar ventas por asesor para este mes
          ...
          console.log(
            `Month Advisor Counts for ${targetDate.getMonth() + 1}/${targetDate.getFullYear()}:`,
            monthAdvisorCounts,
          )
      ...
      console.log("🏆 Historial de ganadores cargado:", winners)
    ...
    return (
      <Card className={cn("", className)}>
        <CardHeader className="pb-3">
          ...
          <CardDescription>Resultados del último mes (coches y motos)</CardDescription>
        </CardHeader>
      ...
      <CardContent className="pb-4">
        <div className="space-y-3">
          {currentRanking.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">No hay datos de ventas este mes</p>
          ) : (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {item.sales_count} ventas • {currencyFormatter.format(item.total_amount)}
                  </p>
                </div>
